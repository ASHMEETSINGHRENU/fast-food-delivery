...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (password_verify($password, $passRow['password'])){
        $sql = "INSERT INTO `tbl_contact` (`userId`, `email`, `phoneNo`, `orderId`, `message`, `time`) VALUES ('$userId', '$email', '$phone', '$orderId', '$message', current_timestamp())";
        $result = mysqli_query($conn, $sql);
        $contactId = $conn->insert_id;
        echo '<script>alert("Thanks for Contact us. Your contact id is ' .$contactId. '. We will contact you very soon.");
                    window.location.href="http://localhost/opd/index.php";  
                    </script>';
                    exit();
    }
    else{
